<@ms.html5>
	...
	<@ms.panel>
			<@shiro.hasPermission name="template:upload"><@ms.uploadFile path="${uploadFileUrl}" uploadFloderPath="true" inputName="file" size="20" filetype="htm,html,jpg,gif,png,css,js,ico,swf,less" msg="建议上传5M以下htm/html/css/js/jpg/gif/png/swf文件"  maxSize="5" callBack="refresh" isRename="false"/></@shiro.hasPermission>
	...
	<script type="text/javascript">
       ...
       function chat() {  
       	  	  ...
       	  	  if($("#file").val().substring($("#file").val().lastIndexOf(".")+1) == "zip"){
	       	  	  $.ajax({
					   type: "post",
					   url: "${managerPath}/template/unZip.do",
					   data: "fileUrl=" + fileUrl,
					   success: function(msg){ 
					     	location.href="${managerPath}/template/showChildFileAndFolder.do?skinFolderName="+temp;
		...
		$("#deleteButton").click(function(){
			...
			var URL = "${managerPath}/template/deleteTemplateFile.do";
	       ...
	       $(".name").each(function(i){
			...
			$("table").delegate(".editFileBtn","click",function(){
		        ...
		        if(fileName.indexOf(".") >= 0){
		        	location.href = "${managerPath}/template/readFileContent.do?fileName="+encodeURIComponent(fileName);
		        } else {
					location.href="${managerPath}/template/showChildFileAndFolder.do?skinFolderName="+encodeURIComponent(fileName);
				}
			});
