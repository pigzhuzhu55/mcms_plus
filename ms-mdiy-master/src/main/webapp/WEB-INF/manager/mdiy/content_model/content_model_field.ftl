		...
		<#switch type>
			...
			<#case "7">
					<@ms.formRow label="${name}">
							<@ms.uploadImg path="${appId}/article" inputName="${filedName}" size="4"   msg="提示：可以上传多张图片"  maxSize="10" imgs="${value?default('')}" />
					</@ms.formRow>
			<#break>
			...
			<#case "8">
				...
				<@ms.formRow >
						<@ms.uploadFile path="${appId}/article"  inputName="${filedName}" size="1"  msg="建议上传5M以下的文件,文件类型支持格式：zip、rar、doc、xls、xlsx、ppt、pptx、docx、txt、pdf " filetype="zip,rar,doc,xls,xlsx,ppt,pptx,docx,txt,pdf" maxSize="5" callBack="setFile${filedName}" isRename="false"/>
				</@ms.formRow>
