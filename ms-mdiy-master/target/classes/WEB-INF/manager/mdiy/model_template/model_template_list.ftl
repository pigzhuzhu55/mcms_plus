...
$(function(){
	//加载选择模块列表
	$(".templateSelect").request({url:"${managerPath}/template/queryTemplateFileForColumn.do",type:"json",method:"get",func:function(msg) {
	...
	$("#addButton").on("click",function(){
		postUrl= "${managerPath}/mdiy/modeltemplate/save.do";
		$("#addEditBtn").text("保存");
	...
	$(".editModel").on("click",function(){
		...
		var actionUrl="${managerPath}/mdiy/modeltemplate/"+tmplModelId+"/edit.do";	//编辑请求地址
		$(".editModel").request({url:actionUrl,type:"json",method:"post",func:function(data) {
			...
			if(model!=null){
				...
				$(".addEditModel").modal();
				postUrl="${managerPath}/mdiy/modeltemplate/"+tmplModelId+"/update.do",
				$("#addEditBtn").text("更新");
	...
	$("#rightDelete").on("click",function(){
		var actionUrl="${managerPath}/mdiy/modeltemplate/"+tmplModelId+"/delete.do";
		$(this).text("删除中");
