<@ms.html5>
	...
	<@ms.panel>
		<@ms.form name="dictForm" isvalidation=true>
			...
			<@ms.text label="数据值" name="dictValue" value="${dictEntity.dictValue?default('')}"  width="360px;" placeholder="请输入数据值" validation={"required":"false","maxlength":"60","data-bv-stringlength-message":"标签名长度不能超过六十个字符长度!", "data-bv-notempty-message":"必填项目"}/>
			<@ms.number label="排序" name="dictSort" value="${dictEntity.dictSort?default('')}" width="360px;" min=0 max=999 maxlength="3" placeholder="请输入排序（升序）" validation={"required":"false", "data-bv-notempty-message":"必填项目"}/>
			...
			<@ms.text label="子业务数据类型" name="isChild" value="${dictEntity.isChild?default('')}"  width="360px;" placeholder="请输入数据类型" validation={"maxlength":"50","data-bv-stringlength-message":"数据类型长度不能超过五十个字符长度!"} help="注：用于子业务扩展类型"/>
